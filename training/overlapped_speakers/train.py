...
def train(run_name, speaker, start_epoch, stop_epoch, img_c, img_w, img_h, frames_n, absolute_max_string_len, minibatch_size):
    ...
    # Train on a fixed cirriculum
    curriculum = Curriculum(curriculum_rules)
    # Generate frames focusing on lip based on the given curriculum and image size
    lip_gen = BasicGenerator(dataset_path=DATASET_DIR,
                                minibatch_size=minibatch_size,
                                img_c=img_c, img_w=img_w, img_h=img_h, frames_n=frames_n,
                                absolute_max_string_len=absolute_max_string_len,
                                curriculum=curriculum, start_epoch=start_epoch,
                                vtype='mouth', face_predictor_path='/home/derek/Desktop/projects/lipread/riz-lipnet/common/predictors/shape_predictor_68_face_landmarks.dat').build()
    # Creating the model
    ...
    print ">> lipnet.model.compile"
    lipnet.model.compile(loss={'ctc': lambda y_true, y_pred: y_pred}, optimizer=adam)

    # load weight if necessary
    print ">> load weight if necessary"
    if start_epoch > 0:
...
if __name__ == '__main__':
    ...
    speaker = sys.argv[1]
    train(run_name, speaker, 0, 5000, 3, 100, 50, 75, 32, 20)